...
import type { ImageToAvifMessages, AvifBatchResult } from "./client/types"

type ImageToAvifClientProps = Readonly<{
  messages: ImageToAvifMessages
}>

function resolveErrorMessage(error: unknown, messages: ImageToAvifMessages) {
...
function ImageToAvifClient({ messages }: ImageToAvifClientProps) {
  const inputId = useId()
  const [files, setFiles] = useState<File[]>([])
  const [options, setOptions] =
    useState<ImageToAvifOptions>(DEFAULT_AVIF_OPTIONS)
  const [batchResult, setBatchResult] = useState<AvifBatchResult | null>(null)
  ...
  async function handleConvert() {
    ...
    const results: AvifBatchResult["results"] = []
    const failures: string[] = []

    try {
      for (const file of files) {
        const outputName = resolveUniqueAvifOutputName(file.name, nameCounts)

        try {
          const result = await convertImageFileToAvif(file, options, outputName)
          results.push(result)
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_24rem]">
        ...
        <OptionsCard
          ...
          onOptionsChange={(nextOptions) => {
            setOptions(normalizeAvifOptions(nextOptions))
          }}
          options={options}
        />
      </div>
...
export default ImageToAvifClient

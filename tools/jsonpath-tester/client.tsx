...
import { evaluateJsonpathText, parseJsonText } from "./core/evaluate-jsonpath"

type ResultMode = "paths" | "values"

type JsonpathTesterClientProps = Readonly<{
  messages: JsonpathTesterMessages
}>

function JsonpathTesterClient({ messages }: JsonpathTesterClientProps) {
  ...
  const [activeResultMode, setActiveResultMode] = useState<ResultMode>("values")
  const [downloadUrl, setDownloadUrl] = useState<string | null>(null)

  const deferredJsonText = useDeferredValue(jsonText)
  const deferredQueryText = useDeferredValue(queryText)
  const evaluation = useMemo(
    () => evaluateJsonpathText(deferredJsonText, deferredQueryText),
  ...
  const exampleOptions = useMemo(
    () => [
      {
        label: messages.exampleAuthorsLabel,
        value: EXAMPLE_QUERY_VALUES.authors,
      },
      {
        label: messages.exampleCheapBooksLabel,
        value: EXAMPLE_QUERY_VALUES.cheapBooks,
      },
      {
        label: messages.exampleBicycleColorLabel,
        value: EXAMPLE_QUERY_VALUES.bicycleColor,
      },
    ],
    [messages]
  )
  ...
  }, [activeResultValue, evaluation.state])
...
export default JsonpathTesterClient

...
import { formatSha1Digest, hashSha1 } from "./sha1"

describe("hashSha1", () => {
  test("hashes a blob with SHA-1", async () => {
    const input = "hello"
    const digest = await hashSha1(
      new Blob([input]),
      webcrypto.subtle satisfies Pick<SubtleCrypto, "digest">
    )
    const expectedHex = createHash("sha1").update(input).digest("hex")

    expect(digest.hex).toBe(expectedHex)
    expect(digest.base64).toBe(
      createHash("sha1").update(input).digest("base64")
    )
  })
  ...
  test("throws when Web Crypto is unavailable", async () => {
    ...
    ).rejects.toThrow("SHA-1 hashing requires Web Crypto support.")
  })
})

describe("formatSha1Digest", () => {
  test("formats a digest into every supported representation", () => {
    const digest = Uint8Array.from([0, 1, 2, 255]).buffer

    expect(formatSha1Digest(digest)).toEqual({
      hex: "000102ff",
      base64: "AAEC/w==",
      decimal: "66303",

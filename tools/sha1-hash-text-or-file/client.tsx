...
import type { Sha1HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashSha1, type Sha1Digest } from "./core/sha1"

type Sha1DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Sha1Digest }
  | { status: "error"; message: string }

type Sha1HashTextOrFileClientProps = Readonly<{
  messages: Sha1HashTextOrFilePageMessages
}>

function Sha1HashTextOrFileClient({ messages }: Sha1HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Sha1DigestState>({
    status: "loading",
  })
...
export default Sha1HashTextOrFileClient

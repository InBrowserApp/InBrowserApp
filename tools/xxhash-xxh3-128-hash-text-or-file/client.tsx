...
import type { XxHashXxh3128HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import {
  hashXxHashXxh3128,
  parseSeedInput,
  type XxHashXxh3128Digest,
} from "./core/xxhash"

type XxHashXxh3128DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: XxHashXxh3128Digest }
  | { status: "error"; message: string }

type XxHashXxh3128HashTextOrFileClientProps = Readonly<{
  messages: XxHashXxh3128HashTextOrFilePageMessages
}>

function XxHashXxh3128HashTextOrFileClient({
  messages,
}: XxHashXxh3128HashTextOrFileClientProps) {
  ...
  const [digestState, setDigestState] = useState<XxHashXxh3128DigestState>({
    status: "loading",
  })
  ...
  useEffect(() => {
    ...
    const currentSeed = parseSeedInput(seedInput)

    if (!currentSeed.isValid) {
      setDigestState({ status: "error", message: messages.seedInvalid })
      return
    }

    setDigestState({ status: "loading" })

    void hashXxHashXxh3128(source, currentSeed)
      .then((digest) => {
        if (!cancelled) {
    ...
    <div className="grid gap-6">
      <Card>
        ...
        <CardContent className="flex flex-1 flex-col gap-4">
          ...
          <Field data-invalid={parsedSeed.isValid ? undefined : true}>
            <FieldLabel htmlFor={seedId}>{messages.seedLabel}</FieldLabel>
            <Input
              id={seedId}
              type="text"
              inputMode="text"
              ...
              aria-invalid={parsedSeed.isValid ? undefined : true}
              onChange={(event) => {
            ...
            />
            <FieldError>
              {parsedSeed.isValid ? null : messages.seedInvalid}
            </FieldError>
          </Field>
        </CardContent>
...
function DigestSection({
  ...
  state: XxHashXxh3128DigestState
  messages: XxHashXxh3128HashTextOrFilePageMessages
}>) {
...
export default XxHashXxh3128HashTextOrFileClient

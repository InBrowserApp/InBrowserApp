...
import Sha512224HashTextOrFileClient from "./client"

const messages = {
  meta: {
    name: "SHA-512/224 (FIPS 180-4) Hash Text or File",
    description:
      "Generate SHA-512/224 (FIPS 180-4) digests directly in your browser.",
  },
  inputLabel: "Input",
...
const STORAGE_KEY = "tools:sha512-224-hash-text-or-file:text"
...
function sha512224Hex(input: string) {
  return createHash("sha512-224").update(input).digest("hex")
}

describe("Sha512224HashTextOrFileClient", () => {
  test("renders the default example and its digest", async () => {
    render(<Sha512224HashTextOrFileClient messages={messages} />)
    ...
    expect(
      await screen.findByText(sha512224Hex("Hello, browser-native world!"))
    ).toBeTruthy()
  })
  ...
  test("updates the text digest when the input changes", async () => {
    ...
    expect(await screen.findByText(sha512224Hex("test"))).toBeTruthy()
  })
  ...
  test("imports a file and renders its digest", async () => {
    ...
    expect(
      await screen.findByText(sha512224Hex("hello from file"))
    ).toBeTruthy()
  })
  ...
  test("restores the last stored text value", async () => {
    ...
    expect(await screen.findByText(sha512224Hex("stored value"))).toBeTruthy()
  })

...
import type { Sha512224HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashSha512224, type Sha512224Digest } from "./core/sha512224"

type Sha512224DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Sha512224Digest }
  | { status: "error"; message: string }

type Sha512224HashTextOrFileClientProps = Readonly<{
  messages: Sha512224HashTextOrFilePageMessages
}>

function Sha512224HashTextOrFileClient({
  messages,
}: Sha512224HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Sha512224DigestState>({
    status: "loading",
  })
...
export default Sha512224HashTextOrFileClient

...
function GroupEditor({
  ...
  function removeUserAgent(index: number) {
    onGroupUserAgentsChange(
      group.id,
      group.userAgents.filter((_, userAgentIndex) => userAgentIndex !== index)
    )
  }

  function updateRule(index: number, nextRule: RobotsGroup["rules"][number]) {
    onGroupRulesChange(
      group.id,
  ...
  return (
    <section className="grid gap-4 rounded-xl border border-border/70 bg-muted/20 p-4">
      ...
      <div className="grid gap-3">
          ...
          {group.rules.map((rule, ruleIndex) => (
            ...
            >
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="allow">{messages.ruleAllow}</SelectItem>
        ...
        <div className="text-sm text-muted-foreground">{messages.ruleHint}</div>
      </div>

...
import type { Sha3256HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashSha3256, type Sha3256Digest } from "./core/sha3256"

type Sha3256DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Sha3256Digest }
  | { status: "error"; message: string }

type Sha3256HashTextOrFileClientProps = Readonly<{
  messages: Sha3256HashTextOrFilePageMessages
}>

function Sha3256HashTextOrFileClient({
  messages,
}: Sha3256HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Sha3256DigestState>({
    status: "loading",
  })
...
export default Sha3256HashTextOrFileClient

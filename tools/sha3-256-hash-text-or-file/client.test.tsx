...
import Sha3256HashTextOrFileClient from "./client"

const messages = {
  meta: {
    name: "SHA3-256 (FIPS 202) Hash Text or File",
    description:
      "Generate SHA3-256 (FIPS 202) digests directly in your browser.",
  },
  inputLabel: "Input",
...
function sha3256Hex(input: string) {
  return createHash("sha3-256").update(input).digest("hex")
}

describe("Sha3256HashTextOrFileClient", () => {
  test("renders the default example and its digest", async () => {
    render(<Sha3256HashTextOrFileClient messages={messages} />)
    ...
    expect(
      await screen.findByText(sha3256Hex("Hello, browser-native world!"))
    ).toBeTruthy()
  })
  ...
  test("updates the text digest when the input changes", async () => {
    ...
    expect(await screen.findByText(sha3256Hex("test"))).toBeTruthy()
  })
  ...
  test("imports a file and renders its digest", async () => {
    ...
    expect(await screen.findByText(sha3256Hex("hello from file"))).toBeTruthy()
  })
  ...
  test("switches back to text mode after importing a file", async () => {
    ...
    await screen.findByText(sha3256Hex("hello from file"))
  ...
  test("restores the last stored text value", async () => {
    ...
    expect(await screen.findByText(sha3256Hex("stored value"))).toBeTruthy()
  })

...
async function hashSipHash24(source: Blob, key: Uint8Array) {
  const data = new Uint8Array(await source.arrayBuffer())

  return formatSipHashDigest(sipHash24Bytes(data, key))
}
...
function sipHash24Bytes(data: Uint8Array, key: Uint8Array) {
  if (key.length !== KEY_LENGTH_BYTES) {
    throw new Error("SipHash-2-4 requires a 16-byte key.")
  }
  ...
  const finalized = sipRounds([v0, v1, v2, v3], 4)
  const result =
    (finalized[0] ^ finalized[1] ^ finalized[2] ^ finalized[3]) & MASK_64
...
function formatSipHashDigest(bytes: Uint8Array): SipHashDigest {
  if (bytes.length !== DIGEST_LENGTH_BYTES) {
    throw new Error("SipHash-2-4 output must be 8 bytes.")
  }
...
function writeUint64BE(value: bigint) {
  ...
  for (let index = 0; index < DIGEST_LENGTH_BYTES; index += 1) {
    output[DIGEST_LENGTH_BYTES - 1 - index] = Number(
...
export { formatSipHashDigest, hashSipHash24, parseSipHashKey, sipHash24Bytes }

...
import * as jwkPemUtils from "./jwk-pem"
import {
  base64UrlToBytes,
  bytesToBase64Url,
  isBase64Input,
...
describe("jwkPem utilities", () => {
  ...
  it("returns JWK Set for multiple PEM blocks", async () => {
    ...
    const rsaPublic = (await webcrypto.subtle.exportKey(
      "jwk",
...
-----END PRIVATE KEY-----`
    ...
    const result = await pemToJwk(
      [
        samplePem,
        wrapPem("PUBLIC KEY", spkiUnknown),
        wrapPem("PRIVATE KEY", pkcs8Unknown),
      ].join("\n")
    )

    const warningKeys = result.warnings.map((warning) => warning.key)
    expect(
      warningKeys.filter((key) => key === "errorUnsupportedAlgorithm")
    ).toHaveLength(2)
  })
  ...
  it("converts PKCS1 RSA PEM blocks", async () => {
    ...
    const pkcs1Private = privateKey
      .export({ format: "pem", type: "pkcs1" })
      .toString()
    const pkcs1Public = publicKey
      .export({ format: "pem", type: "pkcs1" })
      .toString()
  ...
  it("rejects PEM input with no valid blocks", async () => {
    const unsupported = wrapPem("CERTIFICATE", new Uint8Array([1, 2, 3]))
    await expect(pemToJwk(unsupported)).rejects.toMatchObject({
      key: "errorInvalidPem",
    })
  })

  it("rejects invalid PEM text that is not base64", async () => {
    await expect(pemToJwk("---")).rejects.toMatchObject({
      key: "errorInvalidPem",
    })
  })
  ...
  it("records warnings for unexpected conversion errors", async () => {
    const spy = vi
      ...
      .mockReturnValue([
        { label: null as unknown as string, der: new Uint8Array() },
      ])

    await expect(pemToJwk("ignored")).rejects.toMatchObject({
      key: "errorInvalidPem",
    })
...
-----END PRIVATE KEY-----`
  ...
  it("rejects missing key material in JWK inputs", async () => {
    ...
    await expect(
      jwkToPem(
        { kty: "EC", crv: "P-256", x: "a", y: "b" } as JsonWebKey,
        "private"
      )
    ).rejects.toMatchObject({
      key: "errorMissingPrivateKey",
    ...
    await expect(
      jwkToPem({ kty: "OKP" } as JsonWebKey, "public")
    ).rejects.toMatchObject({
      key: "errorMissingField",
    })
    await expect(
  ...
  it("fails to export minimal RSA private keys without CRT params", async () => {
    ...
    await expect(
      jwkToPem(minimalRsa as JsonWebKey, "private")
    ).rejects.toMatchObject({
  ...
  it("covers DER detection and OID helpers", () => {
    expect(testUtils.detectDerKeyType(testUtils.encodeSequence())).toBe(
      "unknown"
    )
    expect(
      testUtils.detectDerKeyType(
        testUtils.encodeSequence(testUtils.encodeInteger(0))
      )
    ).toBe("pkcs8")
    expect(
      testUtils.detectDerKeyType(
        testUtils.encodeSequence(testUtils.encodeSequence())
      )
    ).toBe("spki")
    expect(
      testUtils.detectDerKeyType(
  ...
  it("rejects invalid ASN.1 structures", () => {
    expect(() => testUtils.readAsn1Element(new Uint8Array(), 0)).toThrow(
      JwkPemError
    )
    expect(() => testUtils.readAsn1Element(new Uint8Array([0x30]), 0)).toThrow(
      JwkPemError
    )
    expect(() =>
      testUtils.readAsn1Element(new Uint8Array([0x30, 0x80]), 0)
    ).toThrow(JwkPemError)
    expect(() =>
      testUtils.readAsn1Element(new Uint8Array([0x30, 0x82, 0x01]), 0)
    ).toThrow(JwkPemError)
    expect(() =>
      testUtils.readAsn1Element(new Uint8Array([0x30, 0x02, 0x01]), 0)
    ).toThrow(JwkPemError)

    const invalidChildren = new Uint8Array([0x30, 0x02, 0x02, 0x01, 0x00])
    const parent = testUtils.readAsn1Element(invalidChildren, 0)
    expect(() => testUtils.readAsn1Children(invalidChildren, parent)).toThrow(
      JwkPemError
    )

    expect(() =>
      testUtils.parseAlgorithmIdentifier(new Uint8Array(), undefined)
    ).toThrow(JwkPemError)
    ...
    expect(() =>
      testUtils.parseAlgorithmIdentifier(notSequence, notSequenceElement)
    ).toThrow(JwkPemError)

    const emptySequence = testUtils.encodeSequence()
    const emptyElement = testUtils.readAsn1Element(emptySequence, 0)
    expect(() =>
      testUtils.parseAlgorithmIdentifier(emptySequence, emptyElement)
    ).toThrow(JwkPemError)
    ...
    expect(() =>
      testUtils.parseAlgorithmIdentifier(missingOid, missingOidElement)
    ).toThrow(JwkPemError)
    ...
    expect(
      testUtils.parseAlgorithmIdentifier(nullParams, nullParamsElement)
    ).toEqual({
  ...
  it("validates SPKI, PKCS8, and OKP structures", async () => {
    ...
    expect(() => testUtils.parseSpkiAlgorithm(spkiBadCurve)).toThrow(
      JwkPemError
    )

    const ecNoParams = testUtils.encodeSequence(
      testUtils.encodeOid("1.2.840.10045.2.1")
    )
    ...
    expect(() => testUtils.parseSpkiAlgorithm(spkiNoParams)).toThrow(
      JwkPemError
    )
    ...
    expect(() => testUtils.parsePkcs8Algorithm(pkcs8BadCurve)).toThrow(
      JwkPemError
    )
    ...
    expect(() => testUtils.parsePkcs8Algorithm(pkcs8NoParams)).toThrow(
      JwkPemError
    )
    ...
    await expect(testUtils.parseOkpPkcs8(pkcs8WrongAlg)).rejects.toThrow(
      JwkPemError
    )
    ...
    await expect(testUtils.parseOkpPkcs8(pkcs8BadPrivate)).rejects.toThrow(
      JwkPemError
    )
    ...
    await expect(testUtils.parseOkpPkcs8(pkcs8BadInner)).rejects.toThrow(
      JwkPemError
    )

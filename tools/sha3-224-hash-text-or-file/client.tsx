...
import type { Sha3224HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashSha3224, type Sha3224Digest } from "./core/sha3224"

type Sha3224DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Sha3224Digest }
  | { status: "error"; message: string }

type Sha3224HashTextOrFileClientProps = Readonly<{
  messages: Sha3224HashTextOrFilePageMessages
}>

function Sha3224HashTextOrFileClient({
  messages,
}: Sha3224HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Sha3224DigestState>({
    status: "loading",
  })
...
export default Sha3224HashTextOrFileClient

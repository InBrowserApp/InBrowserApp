...
describe("parseDateTimeInput", () => {
  test("parses valid date time strings", () => {
  ...
  test("parses date-only values with zeroed time parts", () => {
...
describe("time zone conversions", () => {
  test("includes UTC in supported time zones", () => {
  ...
  test("falls back when Intl.supportedValuesOf is missing from Intl", () => {
    const descriptor = Object.getOwnPropertyDescriptor(
      Intl,
      "supportedValuesOf"
    )
  ...
  test("skips preferred time zones that are not actually supported", async () => {
    ...
    try {
      vi.resetModules()
      const isolated = await import("./time-zone")
  ...
  test("formats UTC timestamps in UTC", () => {
    expect(formatInTimeZone(0, "UTC")).toBe("1970-01-01 00:00:00.000")
  })

  test("formats date time parts", () => {
    expect(
      formatDateTimeParts({
        year: 2024,
        month: 1,
  ...
  test("reports unsupported time zones", () => {
    expect(isTimeZoneSupported("Mars/Phobos")).toBe(false)
  })

  test("converts a UTC time to timestamp", () => {
  ...
  test("handles DST gap normalization when converting to UTC", () => {
    const utc = toUtcTimestamp(
      {
        year: 2024,
        month: 3,
  ...
  test("converts Asia/Shanghai to the expected UTC time", () => {
    const utc = toUtcTimestamp(
      {
        year: 2024,
        month: 1,
  ...
  test("formats offsets with sign and padding", () => {

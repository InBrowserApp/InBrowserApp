...
const MAC_STORAGE_KEY = "tools:mac-to-ipv6-link-local:mac"
...
function MacToIpv6LinkLocalClient({ messages }: MacToIpv6LinkLocalClientProps) {
  ...
  useEffect(() => {
    ...
    const storedMac = window.localStorage.getItem(MAC_STORAGE_KEY)
    if (storedMac !== null) {
      setMac(storedMac)
    }

    const storedNetworkInterface = window.localStorage.getItem(
      NETWORK_INTERFACE_STORAGE_KEY
    )

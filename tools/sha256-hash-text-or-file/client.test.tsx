...
import Sha256HashTextOrFileClient from "./client"

const messages = {
  meta: {
    name: "SHA-256 Hash Text or File",
    description: "Generate SHA-256 digests directly in your browser.",
  },
  inputLabel: "Input",
...
describe("Sha256HashTextOrFileClient", () => {
  ...
  test("updates the text digest when the input changes", async () => {
    ...
    expect(await screen.findByText(sha256Hex("test"))).toBeTruthy()
  })
  ...
  test("restores the last stored text value", async () => {
    ...
    expect(await screen.findByText(sha256Hex("stored value"))).toBeTruthy()
  })

...
describe("radio timecode time helpers", () => {
  test("detects leap years and day-of-year", () => {
    expect(isLeapYear(2024)).toBe(true)
    expect(isLeapYear(2100)).toBe(false)
    expect(getDayOfYear(2024, 2, 29)).toBe(60)
  })

  test("handles non-leap years and out-of-range months defensively", () => {
    expect(getDayOfYear(2023, 3, 1)).toBe(60)
    expect(getDayOfYear(2024, 13, 1)).toBe(367)
    expect(getDayOfYear(2024, 14, 1)).toBe(367)
  })

  test("falls back to zeros when Intl parts are missing", () => {
  ...
  test("computes DST status for a UTC day", () => {

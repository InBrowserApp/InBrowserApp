type EmailValidationResult = Readonly<{
  raw: string
  normalized: string
  length: number
  localPart: string
...
function hasValidDotPlacement(value: string) {
  return (
    value.length > 0 &&
    !value.startsWith(".") &&
    !value.endsWith(".") &&
...
function isValidDomainLabel(label: string) {
  if (label.length === 0 || label.length > DOMAIN_LABEL_MAX_LENGTH) {
    return false
  }
...
function validateEmail(input: string): EmailValidationResult {
  ...
  const normalized =
    hasSingleAt && domain ? `${localPart}@${domain.toLowerCase()}` : ""
  const localLength = localPart.length
  const domainLength = domain.length

  const isLengthValid = length <= EMAIL_MAX_LENGTH
  const isLocalLengthValid =
    localLength > 0 && localLength <= LOCAL_PART_MAX_LENGTH
  const isDomainLengthValid =
    domainLength > 0 && domainLength <= DOMAIN_MAX_LENGTH
  const isLocalCharsValid = localLength > 0 && LOCAL_PART_REGEX.test(localPart)
  const isLocalDotsValid = localLength > 0 && hasValidDotPlacement(localPart)
  const isDomainCharsValid = domainLength > 0 && DOMAIN_REGEX.test(domain)
  const isDomainDotsValid = domainLength > 0 && hasValidDotPlacement(domain)
  ...
  const isDomainLabelCharsValid =
    domainLabels.length > 0 &&
    domainLabels.every((label) => isValidDomainLabel(label))
  const tld =
    domainLabels.length > 0 ? domainLabels[domainLabels.length - 1]! : ""
  const isTldValid =
    domainLabels.length > 1 && tld.length >= 2 && TLD_REGEX.test(tld)
...
export { normalizeEmail, validateEmail, type EmailValidationResult }

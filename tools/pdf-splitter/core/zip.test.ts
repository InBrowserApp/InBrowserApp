...
describe("createStoredZipBytes", () => {
  ...
  test("writes local files and central directory records", () => {
    ...
    const bytes = createStoredZipBytes([
      { data: first, name: "alpha.pdf" },
      { data: second, name: "nested\\beta.pdf" },
    ])
    const view = new DataView(bytes.buffer)
  ...
  test("uses a default entry name when a zip entry name is blank", () => {
    const bytes = createStoredZipBytes([
      { data: encoder.encode("pdf"), name: "   " },
    ])

    expect(decoder.decode(bytes)).toContain("document.pdf")
  })
  ...
  test("rejects unsupported zip sizes", () => {
    ...
    expect(() =>
      createStoredZipBytes([{ data: hugeIterable, name: "huge.pdf" }])
    ).toThrowError("ZIP_TOO_LARGE")
  })
})

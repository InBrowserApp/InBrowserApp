...
import type { SriHashGeneratorPageMessages } from "./client/types"
import { SriOutputList } from "./components/sri-output-list"
import { generateSri, type SriDigest } from "./core/sri"

type SriDigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: SriDigest }
  | { status: "error"; message: string }

type SriHashGeneratorClientProps = Readonly<{
  messages: SriHashGeneratorPageMessages
}>

function SriHashGeneratorClient({ messages }: SriHashGeneratorClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<SriDigestState>({
    status: "loading",
  })
          ...
          setDigestState({
            ...
            message:
              error instanceof Error
                ? error.message
                : selectedFile
                  ? "Failed to generate SRI hashes for the file."
                  : "Failed to generate SRI hashes for the text input.",
          })
        }
      })
...
export default SriHashGeneratorClient

...
async function hashSha512(
  ...
  if (!subtle) {
    throw new Error("SHA-512 hashing requires Web Crypto support.")
  }

  const input = await source.arrayBuffer()
  const digest = await subtle.digest("SHA-512", input)

  return formatSha512Digest(digest)
}

function formatSha512Digest(digest: ArrayBuffer): Sha512Digest {
...
export { formatSha512Digest, hashSha512 }
export type { Sha512Digest }

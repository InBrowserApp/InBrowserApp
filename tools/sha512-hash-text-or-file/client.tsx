...
import { hashSha512, type Sha512Digest } from "./core/sha512"

type Sha512DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Sha512Digest }
  | { status: "error"; message: string }

type Sha512HashTextOrFileClientProps = Readonly<{
  messages: Sha512HashTextOrFilePageMessages
}>

function Sha512HashTextOrFileClient({
  messages,
}: Sha512HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Sha512DigestState>({
    status: "loading",
  })
...
export default Sha512HashTextOrFileClient

...
import Sha512HashTextOrFileClient from "./client"

const messages = {
  meta: {
    name: "SHA-512 Hash Text or File",
    description: "Generate SHA-512 digests directly in your browser.",
  },
  inputLabel: "Input",
...
const STORAGE_KEY = "tools:sha512-hash-text-or-file:text"
...
describe("Sha512HashTextOrFileClient", () => {
  test("renders the default example and its digest", async () => {
    render(<Sha512HashTextOrFileClient messages={messages} />)
    ...
    expect(
      await screen.findByText(sha512Hex("Hello, browser-native world!"))
    ).toBeTruthy()
  })
  ...
  test("updates the text digest when the input changes", async () => {
    ...
    expect(await screen.findByText(sha512Hex("test"))).toBeTruthy()
  })
  ...
  test("imports a file and renders its digest", async () => {
    ...
    expect(await screen.findByText(sha512Hex("hello from file"))).toBeTruthy()
  })
  ...
  test("switches back to text mode after importing a file", async () => {
    ...
    await screen.findByText(sha512Hex("hello from file"))
  ...
  test("restores the last stored text value", async () => {
    ...
    expect(await screen.findByText(sha512Hex("stored value"))).toBeTruthy()
  })

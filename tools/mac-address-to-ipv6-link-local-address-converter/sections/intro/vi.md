...
Dùng công cụ này khi bạn cần địa chỉ link-local xác định mà EUI-64 suy ra từ địa chỉ MAC của thiết bị.

### Cách ánh xạ EUI-64 hoạt động

1. Chuẩn hóa địa chỉ MAC thành 48 bit.
2. Đảo `U/L bit` trong byte đầu tiên.
3. Chèn `ff:fe` vào giữa để tạo mã định danh giao diện 64 bit.
4. Thêm tiền tố `fe80::/10`.
...
### Hậu tố giao diện tùy chọn

Chỉ thêm `%eth0`, `%en0` hoặc zone index khác khi lệnh cục bộ cần biết nên dùng giao diện nào.

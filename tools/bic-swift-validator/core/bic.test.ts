...
describe("validateBIC", () => {
  ...
  it("validates BIC-11 codes with primary office branch", () => {
  ...
  it("detects non-primary branches", () => {
    const result = validateBIC("DEUTDEFF500")

    expect(result.isValid).toBe(true)
    expect(result.branchCode).toBe("500")
    expect(result.isPrimaryOffice).toBe(false)
  })
  ...
  it("detects test BICs", () => {
    ...
    expect(result.isPassiveParticipant).toBe(false)
  })
  ...
  it("detects passive participants", () => {
    ...
    expect(result.isTestBIC).toBe(false)
  })

  it("flags invalid length and missing segments", () => {
    const result = validateBIC("DEU")
  ...
  it("flags unknown country codes", () => {
    ...
    expect(result.isCountryCodeValid).toBe(true)
    expect(result.isCountryValid).toBe(false)
    expect(result.isValid).toBe(false)
  })

  it("flags invalid location codes", () => {
    const result = validateBIC("DEUTDE@F")

    expect(result.isLocationCodeValid).toBe(false)
    expect(result.isFormatValid).toBe(false)
    expect(result.isValid).toBe(false)
  })

  it("flags invalid branch codes", () => {
    const result = validateBIC("DEUTDEFF0@0")

    expect(result.isBranchCodeValid).toBe(false)
    expect(result.isFormatValid).toBe(false)
    expect(result.isValid).toBe(false)
  })
})

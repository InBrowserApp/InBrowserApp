...
type StripMetadataFormat = "jpeg" | "png" | "webp"

type StripMetadataResult = Readonly<{
  cleaned: Uint8Array
  removedBytes: number
  format: StripMetadataFormat
}>

function stripImageMetadata(bytes: Uint8Array): StripMetadataResult {
...
function detectImageFormat(bytes: Uint8Array): StripMetadataFormat | "unknown" {
  ...
  if (
    matchesSignature(bytes, RIFF_HEADER) &&
    matchesSignature(bytes.subarray(8), WEBP_HEADER)
  ) {
    return "webp"
  }
...
function isPngMetadataChunk(type: string) {
  return ["tEXt", "zTXt", "iTXt", "eXIf", "iCCP"].includes(type)
}
...
function isWebpMetadataChunk(type: string) {
  return ["EXIF", "XMP ", "ICCP"].includes(type)
}

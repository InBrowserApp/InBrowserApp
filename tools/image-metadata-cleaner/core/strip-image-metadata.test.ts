...
function makePngChunk(type: string, data: number[]) {
...
function makeWebpHeader(totalSize: number) {
...
function makeWebpChunk(type: string, data: number[]) {
...
describe("stripImageMetadata", () => {
  it("removes JPEG metadata segments", () => {
    const jpeg = Uint8Array.from([
      0xff, 0xd8, 0xff, 0xe1, 0x00, 0x06, 0xde, 0xad, 0xbe, 0xef, 0xff, 0xe0,
      0x00, 0x04, 0x00, 0x00, 0xff, 0xda, 0x00, 0x04, 0x00, 0x00, 0x11, 0x22,
      0x33, 0xff, 0xd9,
    ])
  ...
  it("removes JPEG comment segments", () => {
    const jpeg = Uint8Array.from([
      0xff, 0xd8, 0xff, 0xfe, 0x00, 0x04, 0xde, 0xad, 0xff, 0xd9,
    ])
  ...
  it("keeps truncated PNG chunks after the signature", () => {
    ...
    const truncatedChunk = [
      0x00, 0x00, 0x00, 0x20, 0x49, 0x48, 0x44, 0x52, 0xff,
    ]
  ...
  it("throws on unsupported formats", () => {
    expect(() =>
      stripImageMetadata(Uint8Array.from([0x00, 0x01, 0x02]))

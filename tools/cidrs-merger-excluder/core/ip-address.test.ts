import { describe, expect, test } from "vitest"

import {
  createIpAddress,
  parseIpAddress,
  stringifyIpAddress,
} from "./ip-address"

describe("IP address helpers", () => {
  test("parses and stringifies IPv4 addresses", () => {
    const parsed = parseIpAddress("192.168.001.010")

    expect(parsed).not.toBeNull()
    expect(parsed?.family).toBe(4)
    expect(parsed?.value).toBe(3232235786n)
    expect(parsed ? stringifyIpAddress(parsed) : null).toBe("192.168.1.10")
    expect(stringifyIpAddress(createIpAddress(4, 0x0a000001n))).toBe("10.0.0.1")
  })

  test("parses and compresses IPv6 addresses", () => {
    const compressed = parseIpAddress("2001:DB8::1")
    const expanded = parseIpAddress("2001:db8:1:2:3:4:5:6")
    const allZero = parseIpAddress("::")
    const firstZeroRun = parseIpAddress("2001:db8:0:0:1:0:0:1")
    const trailingZeroRun = parseIpAddress("2001:db8:0:0:0:0:0:0")

    expect(compressed ? stringifyIpAddress(compressed) : null).toBe(
      "2001:db8::1"
    )
    expect(expanded ? stringifyIpAddress(expanded) : null).toBe(
      "2001:db8:1:2:3:4:5:6"
    )
    expect(allZero ? stringifyIpAddress(allZero) : null).toBe("::")
    expect(firstZeroRun ? stringifyIpAddress(firstZeroRun) : null).toBe(
      "2001:db8::1:0:0:1"
    )
    expect(trailingZeroRun ? stringifyIpAddress(trailingZeroRun) : null).toBe(
      "2001:db8::"
    )
    expect(stringifyIpAddress(createIpAddress(6, 1n))).toBe("::1")
  })

  test("parses IPv6 addresses with an embedded IPv4 tail", () => {
    const parsed = parseIpAddress("::ffff:192.168.0.1")

    expect(parsed).not.toBeNull()
    expect(parsed?.family).toBe(6)
    expect(parsed ? stringifyIpAddress(parsed) : null).toBe("::ffff:c0a8:1")
  })

  test("rejects malformed addresses", () => {
    expect(parseIpAddress("")).toBeNull()
    expect(parseIpAddress("192.168.1")).toBeNull()
    expect(parseIpAddress("1a.2.3.4")).toBeNull()
    expect(parseIpAddress("256.0.0.1")).toBeNull()
    expect(parseIpAddress("2001::db8::1")).toBeNull()
    expect(parseIpAddress("2001:db8::gggg")).toBeNull()
    expect(parseIpAddress("1:2:3:4:5:6:7")).toBeNull()
    expect(parseIpAddress("1:2:3:4:5:6:7:8::")).toBeNull()
    expect(parseIpAddress("2001:db8:")).toBeNull()
    expect(parseIpAddress("fe80::1%eth0")).toBeNull()
    expect(parseIpAddress("2001:db8:192.168.0.1:1")).toBeNull()
    expect(parseIpAddress("2001:db8:192.168.0.1::")).toBeNull()
    expect(parseIpAddress("::ffff:999.0.0.1")).toBeNull()
  })
})

...
function createIpAddress(family: 4 | 6, value: bigint): ParsedIpAddress {
  if (family === 4) {
...
function stringifyIpAddress(address: ParsedIpAddress): string {
...
function parseIpv6(value: string): readonly number[] | null {
  ...
  const leftSegments =
    left.length === 0 ? [] : parseIpv6Sequence(left.split(":"), false)
  const rightSegments =

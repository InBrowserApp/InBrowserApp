...
import { hashMurmurHash3, parseMurmurHash3Seed } from "./core/murmurhash3"

type MurmurHash3HashTextOrFileClientProps = Readonly<{
  messages: MurmurHash3HashTextOrFilePageMessages
}>

function MurmurHash3HashTextOrFileClient({
  messages,
}: MurmurHash3HashTextOrFileClientProps) {
  ...
  const [digestState, setDigestState] = useState<MurmurHash3DigestState>({
    status: "loading",
  })

  const deferredPlainText = useDeferredValue(plainText)
  const seedState = parseMurmurHash3Seed(seedInput)
  ...
  useEffect(() => {
    ...
    void hashMurmurHash3(source, seedState.value)
      .then((digest) => {
        if (!cancelled) {
...
export default MurmurHash3HashTextOrFileClient

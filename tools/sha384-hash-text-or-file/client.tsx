...
import type { Sha384HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashSha384, type Sha384Digest } from "./core/sha384"

type Sha384DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Sha384Digest }
  | { status: "error"; message: string }

type Sha384HashTextOrFileClientProps = Readonly<{
  messages: Sha384HashTextOrFilePageMessages
}>

function Sha384HashTextOrFileClient({
  messages,
}: Sha384HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Sha384DigestState>({
    status: "loading",
  })
...
export default Sha384HashTextOrFileClient

...
import type { Md4HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashMd4, type Md4Digest } from "./core/md4"

type Md4DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Md4Digest }
  | { status: "error"; message: string }

type Md4HashTextOrFileClientProps = Readonly<{
  messages: Md4HashTextOrFilePageMessages
}>

function Md4HashTextOrFileClient({ messages }: Md4HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Md4DigestState>({
    status: "loading",
  })
...
export default Md4HashTextOrFileClient

...
import type { Ripemd256HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashRipemd256, type Ripemd256Digest } from "./core/ripemd256"

type Ripemd256DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Ripemd256Digest }
  | { status: "error"; message: string }

type Ripemd256HashTextOrFileClientProps = Readonly<{
  messages: Ripemd256HashTextOrFilePageMessages
}>

function Ripemd256HashTextOrFileClient({
  messages,
}: Ripemd256HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Ripemd256DigestState>({
    status: "loading",
  })
...
export default Ripemd256HashTextOrFileClient

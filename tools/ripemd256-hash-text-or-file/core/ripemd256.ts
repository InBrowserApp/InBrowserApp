...
class Ripemd256Hasher extends Hasher32le {
  constructor() {
    super()
    this.state.hash = [
...
async function hashRipemd256(source: Blob) {
  const hasher = new Ripemd256Hasher()

  hasher.update(fromArrayBuffer(await source.arrayBuffer()))

  return formatRipemd256Digest(binaryStringToArrayBuffer(hasher.finalize()))
}

function formatRipemd256Digest(digest: ArrayBuffer): Ripemd256Digest {
...
export { formatRipemd256Digest, hashRipemd256 }
export type { Ripemd256Digest }

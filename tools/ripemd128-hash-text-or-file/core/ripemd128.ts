...
class Ripemd128Hasher extends Hasher32le {
  constructor() {
    super()
  ...
  protected override processBlock(block: number[]) {
    ...
    const nextHash = (this.state.hash[1]! + cl + dr) | 0
    ...
    this.state.hash[0] = nextHash
  }
}

async function hashRipemd128(source: Blob) {
  const hasher = new Ripemd128Hasher()

  hasher.update(fromArrayBuffer(await source.arrayBuffer()))

  return formatRipemd128Digest(binaryStringToArrayBuffer(hasher.finalize()))
}

function formatRipemd128Digest(digest: ArrayBuffer): Ripemd128Digest {
...
function leftRound(index: number, x: number, y: number, z: number) {
  ...
  return (((x & z) | (y & ~z)) + 0x8f1bbcdc) | 0
}

function rightRound(index: number, x: number, y: number, z: number) {
  if (index < 16) return (((x & z) | (y & ~z)) + 0x50a28be6) | 0
  if (index < 32) return (((x | ~y) ^ z) + 0x5c4dd124) | 0
  if (index < 48) return (((x & y) | (~x & z)) + 0x6d703ef3) | 0
...
export { formatRipemd128Digest, hashRipemd128 }
export type { Ripemd128Digest }

...
import type { Ripemd128HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashRipemd128, type Ripemd128Digest } from "./core/ripemd128"

type Ripemd128DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Ripemd128Digest }
  | { status: "error"; message: string }

type Ripemd128HashTextOrFileClientProps = Readonly<{
  messages: Ripemd128HashTextOrFilePageMessages
}>

function Ripemd128HashTextOrFileClient({
  messages,
}: Ripemd128HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Ripemd128DigestState>({
    status: "loading",
  })
...
export default Ripemd128HashTextOrFileClient

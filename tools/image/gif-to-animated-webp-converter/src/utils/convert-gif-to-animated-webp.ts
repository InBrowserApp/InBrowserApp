...
import WebPXMuxWasmUrl from 'webpxmux/dist/webpxmux.wasm?url'
import type { GifToAnimatedWebpOptions, GifToAnimatedWebpResult } from '../types'
...
type WebPXMuxInstance = ReturnType<typeof WebPXMux>
let webpMuxPromise: Promise<WebPXMuxInstance> | null = null

async function getWebpMux() {
  if (!webpMuxPromise) {
    const mux = WebPXMux(WebPXMuxWasmUrl)
    webpMuxPromise = mux.waitRuntime().then(() => mux)
  }
  return webpMuxPromise
}

export async function convertGifToAnimatedWebp(
  file: File,
  ...
  const webpFrames = buildWebpFrames(
    scaledFrames,
    delays,
    outputWidth,
    outputHeight,
...
function scaleFramesIfNeeded(
  ...
  return frames.map((frame) => {
    ...
    return scaledData.data
  })
}

function buildWebpFrames(
  frames: Uint8ClampedArray[],
  delays: number[],
  width: number,
  height: number,
  ...
  const webpFrames: WebpFrame[] = frames.map((frame, index) => ({
    duration: delays[index] ?? MIN_FRAME_DELAY,
    isKeyframe: true,
    rgba: rgbaBytesToUint32(frame),
  }))
  ...
  return {
    ...
    height,
    loopCount,
    bgColor: rgbaToUint32(backgroundColor),
    frames: webpFrames,
  }
}

function rgbaBytesToUint32(frame: Uint8ClampedArray) {
  const output = new Uint32Array(frame.length / 4)
  let outIndex = 0
  for (let i = 0; i < frame.length; i += 4) {
    ...
    const b = frame[i + 2] ?? 0
    const a = frame[i + 3] ?? 0
    output[outIndex] = ((r << 24) | (g << 16) | (b << 8) | a) >>> 0
    outIndex += 1
  }
  return output
}

function rgbaToUint32(color: RgbaColor) {
  return ((color.r << 24) | (color.g << 16) | (color.b << 8) | color.a) >>> 0
}

function resolveLoopCount(bytes: Uint8Array, options: GifToAnimatedWebpOptions) {

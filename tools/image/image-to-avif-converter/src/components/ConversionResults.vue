...
const singleDownloadName = computed(() => singleResult.value?.outputName ?? 'image.avif')
...
function formatSavedText(originalBytes: number, outputBytes: number) {
  ...
  /* v8 ignore next -- the UI never renders totalSavedText when the original total is 0 bytes */

...
describe('barcode reader tool metadata', () => {
  it('exports tool info metadata', () => {
    expect(toolInfo.toolID).toBe('barcode-reader')
    expect(toolInfo.path).toBe('/tools/barcode-reader')
    expect(toolInfo.features).toContain('offline')
    expect(toolInfo.meta.en.name).toContain('Barcode')

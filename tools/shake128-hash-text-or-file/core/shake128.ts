...
async function hashShake128(source: Blob, outputBits = DEFAULT_OUTPUT_BITS) {
  const input = await source.arrayBuffer()
  const digestBytes = shake128(new Uint8Array(input), {
    dkLen: outputBits / 8,
  })
  const digest = new Uint8Array(digestBytes.length)
  digest.set(digestBytes)

  return formatShake128Digest(digest.buffer.slice(0))
}

function formatShake128Digest(digest: ArrayBuffer): Shake128Digest {

...
import type { Shake128HashTextOrFilePageMessages } from "./client/types"
...
type Shake128DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Shake128Digest }
  | { status: "error"; message: string }

type Shake128HashTextOrFileClientProps = Readonly<{
  messages: Shake128HashTextOrFilePageMessages
}>

function Shake128HashTextOrFileClient({
  messages,
}: Shake128HashTextOrFileClientProps) {
  ...
  const [outputLengthInput, setOutputLengthInput] = useState(
    String(DEFAULT_OUTPUT_BITS)
  )
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Shake128DigestState>({
    status: "loading",
  })

  const deferredPlainText = useDeferredValue(plainText)
  const parsedOutputBits = parseOutputBitsInput(outputLengthInput)
  ...
  useEffect(() => {
    ...
    const currentOutputBits = parseOutputBitsInput(outputLengthInput)

    if (!currentOutputBits.isValid) {
      setDigestState({
        status: "error",
        message: messages.outputLengthInvalid,
      })
      return
    }
    ...
    void hashShake128(source, currentOutputBits.value)
      .then((digest) => {
        if (!cancelled) {
    ...
    <div className="grid gap-6">
      <Card>
        ...
        <CardContent className="flex flex-1 flex-col gap-4">
          <Field data-invalid={parsedOutputBits.isValid ? undefined : true}>
            ...
            <Input
              ...
              aria-invalid={parsedOutputBits.isValid ? undefined : true}
              onChange={(event) => {
                setOutputLengthInput(event.target.value)
              }}
              className="font-mono sm:max-w-56"
            />
            <FieldError>
              {parsedOutputBits.isValid ? null : messages.outputLengthInvalid}
            </FieldError>
          </Field>
...
export default Shake128HashTextOrFileClient

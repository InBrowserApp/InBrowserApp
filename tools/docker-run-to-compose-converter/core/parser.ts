...
export function parseDockerRunTokens(tokens: string[]): {
  ...
  if (dockerIndex === -1) {
    return {
      data,
      warnings,
      error: 'Skipped input that does not include "docker run".',
    }
  }
  ...
  if (runIndex === -1) {
    return {
      data,
      warnings,
      error: "Skipped input without a docker run command.",
    }
  }
  ...
  if (!data.image) {
    return {
      data,
      warnings,
      error: "Skipped a docker run command without an image.",
    }
  }

...
describe('XxHashHashTextOrFileView', () => {
  ...
  it('hashes blob content with the default seed', async () => {
    ...
    expect(hex).toBe('df8d09e93f874900a99b8775cc15b6c7')
  })

  it('hashes blob content with a custom seed', async () => {
    const wrapper = mount(XxHashHashTextOrFileView)

    await wrapper.find('.seed-input').setValue('1')
    ...
    const buffer = await hash(new Blob(['hello world']))
    const hex = Buffer.from(buffer).toString('hex')
    expect(hex).toBe('fa3a96b27c8228fc9c199070b6177663')
  })
})

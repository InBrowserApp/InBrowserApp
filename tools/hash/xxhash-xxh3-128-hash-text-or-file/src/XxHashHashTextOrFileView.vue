...
const seedState = computed(() => parseSeed(seedInput.value))
...
const hashFunction = computed(() => {
  const { low, high } = seedState.value
  return async (blob: Blob): Promise<ArrayBuffer> => {
    const stream = blob.stream()
    const reader = stream.getReader()
    const hasher = await createXXHash128(low, high)

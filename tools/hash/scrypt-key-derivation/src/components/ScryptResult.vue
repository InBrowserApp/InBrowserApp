  ...
  <template v-if="showOutput">
    ...
    <ToolSection>
      <n-descriptions label-placement="left" bordered :column="1" content-style="width: 100%">
        <n-descriptions-item>
          <template #label>
            <span class="scrypt-result-label">{{ t('hex') }}</span>
          </template>
          <CopyToClipboardTooltip :content="derivedHex" #="{ copy }">
            <n-text
              code
              class="scrypt-result"
              :class="{ 'scrypt-result-evaluating': evaluating }"
              @click="copy"
            >
              {{ derivedHex || '...' }}
            </n-text>
        ...
        <n-descriptions-item>
          <template #label>
            <span class="scrypt-result-label">{{ t('base64') }}</span>
          </template>
...
import { bytesToBase64, bytesToHex, deriveScrypt } from '../utils'
...
const showOutput = computed(() => {
  return (
    ...
    Boolean(props.salt) &&
    !props.saltErrorType &&
    props.costFactorValid &&
    props.blockSizeValid &&
    props.parallelismValid &&
    props.lengthValid
  )
})

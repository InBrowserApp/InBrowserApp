...
const digestMock = vi.fn(() => 0x01020304)
const create32Mock = vi.fn(() => ({
  update: updateMock,
  digest: digestMock,
}))

vi.mock('xxhash-wasm', () => ({
  default: () => Promise.resolve({ create32: create32Mock }),
}))
...
describe('XxHashHashTextOrFileView', () => {
  ...
  it('hashes blob content using XXH32 and returns big-endian output', async () => {
    const wrapper = mount(XxHashHashTextOrFileView)
    ...
    expect(create32Mock).toHaveBeenCalled()

...
let SipHash128_2_4HashTextOrFileView: typeof import('./SipHash128_2_4HashTextOrFileView.vue').default

beforeAll(async () => {
  SipHash128_2_4HashTextOrFileView = (await import('./SipHash128_2_4HashTextOrFileView.vue'))
    .default
})

describe('SipHash128_2_4HashTextOrFileView', () => {
  it('renders the key input and hash template', () => {
    const wrapper = mount(SipHash128_2_4HashTextOrFileView)
  ...
  it('hashes data to a 128-bit output', async () => {
    ...
    const buffer2 = await hash(new Blob([new Uint8Array()]))
    const hex2 = Buffer.from(buffer2).toString('hex')

...
describe('WhatIsSipHash', () => {
  it('renders the SipHash-128-2-4 description', () => {
    const wrapper = mount(WhatIsSipHash)

    const description = wrapper.find('.description')
    expect(description.attributes('data-title')).toBe('What is SipHash-128-2-4?')
    expect(description.attributes('data-description')).toContain('SipHash-128-2-4')
  })
})

...
let MurmurHash3X86_32HashTextOrFileView: typeof import('./MurmurHash3X86_32HashTextOrFileView.vue').default

beforeAll(async () => {
  MurmurHash3X86_32HashTextOrFileView = (await import('./MurmurHash3X86_32HashTextOrFileView.vue'))
    .default
})

describe('MurmurHash3X86_32HashTextOrFileView', () => {
  it('renders the seed input and hash template', () => {
    const wrapper = mount(MurmurHash3X86_32HashTextOrFileView)
  ...
  it('hashes blob content with the default seed', async () => {
    ...
    const value = new DataView(buffer).getUint32(0, false)
    expect(value).toBe(1586663183)
  })
})

...
import WhatIsMurmurHash3 from './WhatIsMurmurHash3.vue'
...
const toUint32 = (value: bigint) => BigInt.asUintN(32, value)

const parseMurmurHash3Seed = (input: string) => {
  ...
  return { value: Number(toUint32(parsed)), isValid: true }
}
...
const seedState = computed(() => parseMurmurHash3Seed(seedInput.value))
...
const hashFunction = computed(() => {
  ...
  return async (blob: Blob): Promise<ArrayBuffer> => {
    ...
    const hash = murmurHash3.x64.hash128(bytes, seed) as string
    return hexToBytes(hash).buffer
  }
})
</script>
...
<i18n lang="json">
{
  ...
  "no": {
    ...
    "seed-placeholder": "0 eller 0x...",
    "seed-invalid": "Skriv inn et desimaltall eller en 0x-hexverdi."
  }
}
</i18n>

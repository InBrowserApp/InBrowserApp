...
describe('WhatIsXxHash', () => {
  it('renders the XXH3 64-bit description', () => {
    const wrapper = mount(WhatIsXxHash)

    const description = wrapper.find('.description')
    expect(description.attributes('data-title')).toBe('What is xxHash (XXH3 64)?')
    expect(description.attributes('data-description')).toContain('XXH3 64')
  })
})

...
function shake256Hex(input: string, outputBits = 512) {
...
describe("Shake256HashTextOrFileClient", () => {
  test("renders the default example and its digest", async () => {
    ...
    expect(getOutputLengthInput().value).toBe("512")
  ...
  test("updates the digest when the output length changes", async () => {
    ...
    expect(
      await screen.findByText(shake256Hex("Hello, browser-native world!", 1024))
    ).toBeTruthy()
  })

...
import { INVALID_BLAKE3_KEY_BASE64_ERROR, hashBlake3 } from "./core/blake3"

type Blake3HashTextOrFileClientProps = Readonly<{
  messages: Blake3HashTextOrFilePageMessages
}>

function Blake3HashTextOrFileClient({
  messages,
}: Blake3HashTextOrFileClientProps) {
  ...
  const [digestState, setDigestState] = useState<Blake3DigestState>({
    status: "loading",
  })
...
function resolveDigestError(
  ...
  if (
    ...
    error.message === INVALID_BLAKE3_KEY_BASE64_ERROR
  ) {
    return {
      status: "error",
...
export default Blake3HashTextOrFileClient

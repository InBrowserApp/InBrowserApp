...
import type { Sm3HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashSm3, type Sm3Digest } from "./core/sm3"

type Sm3DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Sm3Digest }
  | { status: "error"; message: string }

type Sm3HashTextOrFileClientProps = Readonly<{
  messages: Sm3HashTextOrFilePageMessages
}>

function Sm3HashTextOrFileClient({ messages }: Sm3HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Sm3DigestState>({
    status: "loading",
  })
...
export default Sm3HashTextOrFileClient

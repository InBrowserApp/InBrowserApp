...
import Sm3HashTextOrFileClient from "./client"

const messages = {
  meta: {
    name: "SM3 Hash Text or File",
    description: "Generate SM3 digests directly in your browser.",
  },
  inputLabel: "Input",
...
function sm3Hex(input: string) {
  return createHash("sm3").update(input).digest("hex")
}

describe("Sm3HashTextOrFileClient", () => {
  test("renders the default example and its digest", async () => {
    render(<Sm3HashTextOrFileClient messages={messages} />)
    ...
    expect(
      await screen.findByText(sm3Hex("Hello, browser-native world!"))
    ).toBeTruthy()
  })
  ...
  test("updates the text digest when the input changes", async () => {
    ...
    expect(await screen.findByText(sm3Hex("test"))).toBeTruthy()
  })
  ...
  test("imports a file and renders its digest", async () => {
    ...
    expect(await screen.findByText(sm3Hex("hello from file"))).toBeTruthy()
  })
  ...
  test("switches back to text mode after importing a file", async () => {
    ...
    await screen.findByText(sm3Hex("hello from file"))
  ...
  test("restores the last stored text value", async () => {
    ...
    expect(await screen.findByText(sm3Hex("stored value"))).toBeTruthy()
  })

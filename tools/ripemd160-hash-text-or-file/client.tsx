...
import type { Ripemd160HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashRipemd160, type Ripemd160Digest } from "./core/ripemd160"

type Ripemd160DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Ripemd160Digest }
  | { status: "error"; message: string }

type Ripemd160HashTextOrFileClientProps = Readonly<{
  messages: Ripemd160HashTextOrFilePageMessages
}>

function Ripemd160HashTextOrFileClient({
  messages,
}: Ripemd160HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Ripemd160DigestState>({
    status: "loading",
  })
...
export default Ripemd160HashTextOrFileClient

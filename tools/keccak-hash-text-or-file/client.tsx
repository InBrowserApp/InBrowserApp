...
type KeccakDigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: KeccakDigest }
  | { status: "error"; message: string }

type KeccakHashTextOrFileClientProps = Readonly<{
  messages: KeccakHashTextOrFilePageMessages
}>

function KeccakHashTextOrFileClient({
  messages,
}: KeccakHashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [outputLength, setOutputLength] = useState<KeccakOutputLength>(
    DEFAULT_OUTPUT_LENGTH
  )
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<KeccakDigestState>({
    status: "loading",
  })
  ...
  useEffect(() => {
    ...
    void hashKeccak(source, { outputLength })
      .then((digest) => {
        if (!cancelled) {
  ...
  }, [deferredPlainText, outputLength, selectedFile])
...
export default KeccakHashTextOrFileClient

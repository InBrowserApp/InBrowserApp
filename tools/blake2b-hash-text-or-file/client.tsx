...
import { INVALID_BLAKE2B_KEY_BASE64_ERROR, hashBlake2b } from "./core/blake2b"

type Blake2bHashTextOrFileClientProps = Readonly<{
  messages: Blake2bHashTextOrFilePageMessages
}>

function Blake2bHashTextOrFileClient({
  messages,
}: Blake2bHashTextOrFileClientProps) {
  ...
  const [digestState, setDigestState] = useState<Blake2bDigestState>({
    status: "loading",
  })
...
function resolveDigestError(
  ...
  if (
    ...
    error.message === INVALID_BLAKE2B_KEY_BASE64_ERROR
  ) {
    return {
      status: "error",
...
export default Blake2bHashTextOrFileClient

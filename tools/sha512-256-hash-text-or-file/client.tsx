...
import type { Sha512256HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashSha512256, type Sha512256Digest } from "./core/sha512256"

type Sha512256DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Sha512256Digest }
  | { status: "error"; message: string }

type Sha512256HashTextOrFileClientProps = Readonly<{
  messages: Sha512256HashTextOrFilePageMessages
}>

function Sha512256HashTextOrFileClient({
  messages,
}: Sha512256HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Sha512256DigestState>({
    status: "loading",
  })
...
export default Sha512256HashTextOrFileClient

...
import Sha512256HashTextOrFileClient from "./client"

const messages = {
  meta: {
    name: "SHA-512/256 (FIPS 180-4) Hash Text or File",
    description:
      "Generate SHA-512/256 (FIPS 180-4) digests directly in your browser.",
  },
  inputLabel: "Input",
...
const STORAGE_KEY = "tools:sha512-256-hash-text-or-file:text"
...
function sha512256Hex(input: string) {
  return createHash("sha512-256").update(input).digest("hex")
}

describe("Sha512256HashTextOrFileClient", () => {
  test("renders the default example and its digest", async () => {
    render(<Sha512256HashTextOrFileClient messages={messages} />)
    ...
    expect(
      await screen.findByText(sha512256Hex("Hello, browser-native world!"))
    ).toBeTruthy()
  })
  ...
  test("updates the text digest when the input changes", async () => {
    ...
    expect(await screen.findByText(sha512256Hex("test"))).toBeTruthy()
  })
  ...
  test("imports a file and renders its digest", async () => {
    ...
    expect(
      await screen.findByText(sha512256Hex("hello from file"))
    ).toBeTruthy()
  })
  ...
  test("restores the last stored text value", async () => {
    ...
    expect(await screen.findByText(sha512256Hex("stored value"))).toBeTruthy()
  })

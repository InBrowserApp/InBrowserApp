  ...
  meta: {
    ...
    zh: {
      ...
      ui: {
        ...
        copy: '复制',
        whatIsTitle: '什么是罗马数字？',
        whatIsDescription: '罗马数字是起源于古罗马的数字系统，使用拉丁字母来表示数字。基本符号是 I (1)、V (5)、X (10)、L (50)、C (100)、D (500) 和 M (1000)。数字通过按特定规则组合这些符号形成：符号通常相加（VI = 6），但当较小的数字出现在较大数字之前时，则相减（IV = 4）。这个系统在罗马帝国和中世纪欧洲广泛使用，至今仍用于装饰目的、钟表面、电影版权日期和编号列表。',
      }
    },
    'zh-CN': {
      name: '罗马数字 ↔ 阿拉伯数字转换器',
    ...
    'zh-TW': {
      ...
      ui: {
        ...
        copy: '複製',
        whatIsTitle: '什麼是羅馬數字？',
        whatIsDescription: '羅馬數字是起源於古羅馬的數字系統，使用拉丁字母來表示數字。基本符號是 I (1)、V (5)、X (10)、L (50)、C (100)、D (500) 和 M (1000)。數字通過按特定規則組合這些符號形成：符號通常相加（VI = 6），但當較小的數字出現在較大數字之前時，則相減（IV = 4）。這個系統在羅馬帝國和中世紀歐洲廣泛使用，至今仍用於裝飾目的、鐘錶面、電影版權日期和編號列表。',
      }
    },
    'zh-HK': {
      name: '羅馬數字 ↔ 阿拉伯數字轉換器',
    ...
    ja: {
      ...
      ui: {
        ...
        copy: 'コピー',
        whatIsTitle: 'ローマ数字とは？',
        whatIsDescription: 'ローマ数字は古代ローマに起源を持つ記数法で、ラテン文字を使って数を表現します。基本記号は I (1)、V (5)、X (10)、L (50)、C (100)、D (500)、M (1000) です。数字は特定のルールに従ってこれらの記号を組み合わせて作られます：記号は通常足し算されます（VI = 6）が、小さい数字が大きい数字の前に現れる場合は引き算されます（IV = 4）。このシステムはローマ帝国と中世ヨーロッパで広く使用され、今日でも装飾目的、時計の文字盤、映画の著作権日付、番号付きリストに使用されています。',
      }
    },
    ko: {
      name: '아라비아 ↔ 로마 숫자 변환기',
      ...
      ui: {
        ...
        copy: '복사',
        whatIsTitle: '로마 숫자란?',
        whatIsDescription: '로마 숫자는 고대 로마에서 시작된 수 체계로, 라틴 알파벳의 문자를 사용하여 숫자를 나타냅니다. 기본 기호는 I (1), V (5), X (10), L (50), C (100), D (500), M (1000)입니다. 숫자는 특정 규칙에 따라 이러한 기호를 조합하여 형성됩니다: 기호는 일반적으로 더해지지만 (VI = 6), 작은 숫자가 큰 숫자 앞에 나타날 때는 빼집니다 (IV = 4). 이 시스템은 로마 제국과 중세 유럽에서 널리 사용되었으며, 오늘날에도 장식용, 시계 문자판, 영화 저작권 날짜, 번호 매기기 목록에 계속 사용됩니다.',
      }
    },
    ru: {
    ...
    pt: {
      ...
      ui: {
        ...
        whatIsDescription: 'Os números romanos são um sistema numérico originário da Roma antiga, usando letras do alfabeto latino para representar números. Os símbolos básicos são I (1), V (5), X (10), L (50), C (100), D (500) e M (1000). Os números são formados combinando esses símbolos de acordo com regras específicas: os símbolos são tipicamente somados (VI = 6), mas quando um numeral menor aparece antes de um maior, ele é subtraído (IV = 4). Este sistema foi amplamente usado no Império Romano e na Europa medieval, e continua sendo usado hoje para propósitos decorativos, faces de relógios, datas de direitos autorais de filmes e listas numeradas.',
      }
    },
    ar: {
    ...
    nl: {
      ...
      ui: {
        ...
        whatIsDescription: 'Romeinse cijfers zijn een cijfersysteem dat zijn oorsprong vindt in het oude Rome, waarbij letters van het Latijnse alfabet worden gebruikt om getallen weer te geven. De basissymbolen zijn I (1), V (5), X (10), L (50), C (100), D (500) en M (1000). Getallen worden gevormd door deze symbolen te combineren volgens specifieke regels: symbolen worden gewoonlijk opgeteld (VI = 6), maar wanneer een kleiner cijfer voor een groter cijfer verschijnt, wordt het afgetrokken (IV = 4). Dit systeem werd veel gebruikt in het Romeinse Rijk en het middeleeuwse Europa, en wordt vandaag de dag nog steeds gebruikt voor decoratieve doeleinden, klokgezichten, film auteursrechtdata en genummerde lijsten.',
      }
    },
    sv: {
    ...
    th: {
      ...
      ui: {
        ...
        whatIsDescription: 'ตัวเลขโรมันเป็นระบบตัวเลขที่มีต้นกำเนิดในโรมโบราณ ใช้ตัวอักษรจากอักษรละตินเพื่อแสดงตัวเลข สัญลักษณ์พื้นฐานคือ I (1), V (5), X (10), L (50), C (100), D (500) และ M (1000) ตัวเลขเกิดจากการรวมสัญลักษณ์เหล่านี้ตามกฎเฉพาะ: สัญลักษณ์มักจะบวกกัน (VI = 6) แต่เมื่อตัวเลขที่เล็กกว่าปรากฏก่อนตัวเลขที่ใหญ่กว่า จะถูกลบ (IV = 4) ระบบนี้ถูกใช้อย่างแพร่หลายในจักรวรรดิโรมันและยุโรปในยุคกลาง และยังคงใช้ในปัจจุบันสำหรับวัตถุประสงค์ในการตกแต่ง หน้าปัดนาฬิกา วันที่ลิขสิทธิ์ภาพยนตร์ และรายการที่มีหมายเลข',
      }
    },
    id: {
    ...
    no: {
      ...
      ui: {
        ...
        whatIsDescription: 'Romerske tall er et tallsystem som stammer fra antikkens Roma, som bruker bokstaver fra det latinske alfabetet for å representere tall. Grunnsymbolene er I (1), V (5), X (10), L (50), C (100), D (500) og M (1000). Tall dannes ved å kombinere disse symbolene i henhold til spesifikke regler: symboler legges vanligvis sammen (VI = 6), men når et mindre tall vises før et større, trekkes det fra (IV = 4). Dette systemet var mye brukt i Romerriket og middelalderens Europa, og brukes fortsatt i dag til dekorative formål, urskiver, film copyright-datoer og nummererte lister.',
      }
    },
  },

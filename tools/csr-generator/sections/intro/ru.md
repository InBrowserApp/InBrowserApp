...
- **Источник ключа** — выберите *Создать новый* для создания новой ключевой пары или *Импортировать существующий*, чтобы вставить незашифрованный закрытый ключ PKCS#8 PEM. Зашифрованные ключи, устаревшие блоки `RSA PRIVATE KEY` и `EC PRIVATE KEY` не принимаются; сначала преобразуйте их с помощью `openssl pkcs8 -topk8 -nocrypt`.

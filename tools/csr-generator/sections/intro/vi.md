...
- **Nguồn khóa** — chọn *Tạo mới* để tạo cặp khóa mới, hoặc *Nhập sẵn có* để dán khóa PKCS#8 PEM không mã hóa. Không chấp nhận khóa đã mã hóa, `RSA PRIVATE KEY` kiểu cũ và khối `EC PRIVATE KEY`; hãy chuyển đổi chúng bằng `openssl pkcs8 -topk8 -nocrypt` trước.

...
} as const satisfies Record<PageSizePreset, PageDimensions>

function mmToPt(value: number) {
  return value * PT_PER_MM
}

function getBasePageDimensions(pageSize: PageSizePreset): PageDimensions {
...
function getImagePlacement({
  ...
  const widthScale = contentWidth / Math.max(1, imageWidth)
  const heightScale = contentHeight / Math.max(1, imageHeight)
  const scale =
    fitMode === "cover"

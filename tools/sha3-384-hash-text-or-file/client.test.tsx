...
import Sha3384HashTextOrFileClient from "./client"

const messages = {
  meta: {
    name: "SHA3-384 (FIPS 202) Hash Text or File",
    description:
      "Generate SHA3-384 (FIPS 202) digests directly in your browser.",
  },
  inputLabel: "Input",
...
function sha3384Hex(input: string) {
  return createHash("sha3-384").update(input).digest("hex")
}

describe("Sha3384HashTextOrFileClient", () => {
  test("renders the default example and its digest", async () => {
    render(<Sha3384HashTextOrFileClient messages={messages} />)
    ...
    expect(
      await screen.findByText(sha3384Hex("Hello, browser-native world!"))
    ).toBeTruthy()
  })
  ...
  test("updates the text digest when the input changes", async () => {
    ...
    expect(await screen.findByText(sha3384Hex("test"))).toBeTruthy()
  })
  ...
  test("imports a file and renders its digest", async () => {
    ...
    expect(await screen.findByText(sha3384Hex("hello from file"))).toBeTruthy()
  })
  ...
  test("switches back to text mode after importing a file", async () => {
    ...
    await screen.findByText(sha3384Hex("hello from file"))
  ...
  test("restores the last stored text value", async () => {
    ...
    expect(await screen.findByText(sha3384Hex("stored value"))).toBeTruthy()
  })

...
import type { Sha3384HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashSha3384, type Sha3384Digest } from "./core/sha3384"

type Sha3384DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Sha3384Digest }
  | { status: "error"; message: string }

type Sha3384HashTextOrFileClientProps = Readonly<{
  messages: Sha3384HashTextOrFilePageMessages
}>

function Sha3384HashTextOrFileClient({
  messages,
}: Sha3384HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Sha3384DigestState>({
    status: "loading",
  })
...
export default Sha3384HashTextOrFileClient

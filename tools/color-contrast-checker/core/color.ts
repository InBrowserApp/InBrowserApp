...
const NAMED_COLORS = colorNames as Record<
  string,
...
export function parseHslColor(input: string): RGBA | null {
  ...
  const [hueValue, saturationValue, lightnessValue] = parts as [
    string,
    string,
    string,
  ]
...
export function rgbaToHex(rgba: RGBA, includeAlpha = false): string {
  ...
  const rgbHex = convert.rgb.hex(r, g, b).toUpperCase()
...
export function blendOver(foreground: RGBA, background: RGBA): RGBA {
  ...
  const r =
    (clampChannel(foreground.r) * fgAlpha +
      clampChannel(background.r) * bgAlpha * (1 - fgAlpha)) /
    outAlpha
  const g =
    (clampChannel(foreground.g) * fgAlpha +
      clampChannel(background.g) * bgAlpha * (1 - fgAlpha)) /
    outAlpha
  const b =
    (clampChannel(foreground.b) * fgAlpha +
      clampChannel(background.b) * bgAlpha * (1 - fgAlpha)) /
    outAlpha
...
export function resolveContrastColors(
  ...
  background: RGBA,
  canvas: RGBA = WHITE
): Readonly<{ foreground: RGBA; background: RGBA }> {
...
export function contrastRatio(
  ...
  const foregroundLuminance = relativeLuminance(resolved.foreground)
  const backgroundLuminance = relativeLuminance(resolved.background)
...
function splitFunctionalArgs(body: string): Readonly<{
  parts: string[]
  alpha: string | null
}> {

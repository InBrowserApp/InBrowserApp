...
const messages = {
  ...
  encodedOutputLabel: "Hex Encoded",
  encodedOutputEmptyDescription: "Hex output will appear here...",
  downloadFileLabel: "Download Hex",
...
describe("Base16EncoderClient", () => {
  ...
  test("imports a selected file and derives a .hex filename", async () => {
    render(<Base16EncoderClient messages={messages} />)
  ...
  test("falls back to file.hex when the uploaded file name is missing", async () => {
    render(<Base16EncoderClient messages={messages} />)
  ...
  test("ignores stale successful file reads after a newer file is selected", async () => {
    ...
    expect(getOutput().textContent).not.toContain("666F6F")
  })
  ...
  test("restores the last stored input value", async () => {
    ...
    expect(getOutput().textContent).toContain("74657374")
  })
})

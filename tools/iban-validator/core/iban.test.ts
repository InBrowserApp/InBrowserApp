...
describe("formatIBAN", () => {
  it("groups characters in blocks of 4", () => {
    expect(formatIBAN("GB29NWBK60161331926819")).toBe(
...
describe("validateIBAN", () => {
  ...
  it("handles inputs that are too short to include a full country prefix", () => {
    const result = validateIBAN("G")

    expect(result.countryCode).toBeNull()
    expect(result.checkDigits).toBeNull()
  ...
  it("builds structure regexes for all format tokens", () => {
    ...
    const temporarySpecs = {
      ...
      XE: { length: 4, structure: "", example: "XE00" },
    }
    ...
    try {
      ...
      expect(validateIBAN("XE00").isCountryValid).toBe(true)
    } finally {

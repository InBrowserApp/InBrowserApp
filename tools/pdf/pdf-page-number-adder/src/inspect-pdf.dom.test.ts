...
describe('inspect-pdf', () => {
  ...
  it('returns page count for valid PDF', async () => {
    ...
    await expect(
      inspectPdf(new File(['dummy'], 'sample.pdf', { type: 'application/pdf' })),
    ).resolves.toEqual({
      pageCount: 9,
    })
  })
  ...
  it('maps encrypted PDF error', async () => {
    const error = new Error('enc')
    error.name = 'EncryptedPDFError'
    loadMock.mockRejectedValueOnce(error)

    await expect(
      inspectPdf(new File(['dummy'], 'enc.pdf', { type: 'application/pdf' })),
  ...
  it('maps invalid PDF error', async () => {
    ...
    await expect(
      inspectPdf(new File(['dummy'], 'broken.pdf', { type: 'application/pdf' })),

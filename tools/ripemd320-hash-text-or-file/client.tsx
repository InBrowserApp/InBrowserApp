...
import type { Ripemd320HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashRipemd320, type Ripemd320Digest } from "./core/ripemd320"

type Ripemd320DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Ripemd320Digest }
  | { status: "error"; message: string }

type Ripemd320HashTextOrFileClientProps = Readonly<{
  messages: Ripemd320HashTextOrFilePageMessages
}>

function Ripemd320HashTextOrFileClient({
  messages,
}: Ripemd320HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Ripemd320DigestState>({
    status: "loading",
  })
...
export default Ripemd320HashTextOrFileClient

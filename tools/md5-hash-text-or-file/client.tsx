...
import type { Md5HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashMd5, type Md5Digest } from "./core/md5"

type Md5DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Md5Digest }
  | { status: "error"; message: string }

type Md5HashTextOrFileClientProps = Readonly<{
  messages: Md5HashTextOrFilePageMessages
}>

function Md5HashTextOrFileClient({ messages }: Md5HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Md5DigestState>({
    status: "loading",
  })
...
export default Md5HashTextOrFileClient

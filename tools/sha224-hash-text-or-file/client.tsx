...
import { hashSha224, type Sha224Digest } from "./core/sha224"

type Sha224DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Sha224Digest }
  | { status: "error"; message: string }

type Sha224HashTextOrFileClientProps = Readonly<{
  messages: Sha224HashTextOrFilePageMessages
}>

function Sha224HashTextOrFileClient({
  messages,
}: Sha224HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Sha224DigestState>({
    status: "loading",
  })
...
export default Sha224HashTextOrFileClient

...
import Sha3512HashTextOrFileClient from "./client"

const messages = {
  meta: {
    name: "SHA3-512 (FIPS 202) Hash Text or File",
    description:
      "Generate SHA3-512 (FIPS 202) digests directly in your browser.",
  },
  inputLabel: "Input",
...
function sha3512Hex(input: string) {
  return createHash("sha3-512").update(input).digest("hex")
}

describe("Sha3512HashTextOrFileClient", () => {
  test("renders the default example and its digest", async () => {
    render(<Sha3512HashTextOrFileClient messages={messages} />)
    ...
    expect(
      await screen.findByText(sha3512Hex("Hello, browser-native world!"))
    ).toBeTruthy()
  })
  ...
  test("updates the text digest when the input changes", async () => {
    ...
    expect(await screen.findByText(sha3512Hex("test"))).toBeTruthy()
  })
  ...
  test("imports a file and renders its digest", async () => {
    ...
    expect(await screen.findByText(sha3512Hex("hello from file"))).toBeTruthy()
  })
  ...
  test("switches back to text mode after importing a file", async () => {
    ...
    await screen.findByText(sha3512Hex("hello from file"))
  ...
  test("restores the last stored text value", async () => {
    ...
    expect(await screen.findByText(sha3512Hex("stored value"))).toBeTruthy()
  })

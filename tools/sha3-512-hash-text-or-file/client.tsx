...
import type { Sha3512HashTextOrFilePageMessages } from "./client/types"
import { HashOutputGrid } from "./components/hash-output-grid"
import { hashSha3512, type Sha3512Digest } from "./core/sha3512"

type Sha3512DigestState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "ready"; digest: Sha3512Digest }
  | { status: "error"; message: string }

type Sha3512HashTextOrFileClientProps = Readonly<{
  messages: Sha3512HashTextOrFilePageMessages
}>

function Sha3512HashTextOrFileClient({
  messages,
}: Sha3512HashTextOrFileClientProps) {
  const plainTextId = useId()
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  const [plainText, setPlainText] = useState(DEFAULT_TEXT)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [digestState, setDigestState] = useState<Sha3512DigestState>({
    status: "loading",
  })
...
export default Sha3512HashTextOrFileClient

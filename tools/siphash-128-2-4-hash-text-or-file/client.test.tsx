...
import SipHash128HashTextOrFileClient from "./client"
import { hashSipHash128, parseSipHashKey } from "./core/siphash"

const messages = {
  meta: {
    name: "SipHash-128-2-4 Hash Text or File",
    description: "Generate SipHash-128-2-4 keyed hashes in your browser.",
  },
  ...
  emptyStateDescription:
    "Enter a valid 16-byte key and provide text or import a file to generate a SipHash-128-2-4 result.",
...
async function expectedHex(text: string, keyInput: string) {
  ...
  return (await hashSipHash128(new Blob([text]), keyState.key)).hex
}

describe("SipHash128HashTextOrFileClient", () => {
  test("renders the default example and its SipHash-128-2-4 output", async () => {
    render(<SipHash128HashTextOrFileClient messages={messages} />)
  ...
  test("persists key and text edits", () => {
    render(<SipHash128HashTextOrFileClient messages={messages} />)

    fireEvent.change(getKeyInput(), {
      target: { value: "0f0e0d0c0b0a09080706050403020100" },
    })
    fireEvent.change(getPlainTextInput(), {
      target: { value: "persisted payload" },
    })

    expect(window.localStorage.getItem(STORAGE_KEYS.key)).toBe(
      "0f0e0d0c0b0a09080706050403020100"
    )
    expect(window.localStorage.getItem(STORAGE_KEYS.text)).toBe(
      "persisted payload"
    )
  })
})

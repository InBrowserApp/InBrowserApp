...
describe("UuidNilGeneratorClient", () => {
  test("renders the nil UUID representations and reference details", () => {
    ...
    for (const value of [NIL_UUID, NIL_UUID_HEX, NIL_UUID_URN]) {
      ...
      expect(code.className).toContain("sm:whitespace-nowrap")
    }
  })
  ...
  test("keeps representation actions constrained for narrow layouts", () => {
    render(<UuidNilGeneratorClient messages={messages} />)

    const copyHexButton = screen.getByRole("button", {
      name: messages.copyHexLabel,
    })

    expect(copyHexButton.parentElement?.className).toContain("grid")
    expect(copyHexButton.className).toContain("max-w-full")
    expect(copyHexButton.className).toContain("whitespace-normal")
  })
})

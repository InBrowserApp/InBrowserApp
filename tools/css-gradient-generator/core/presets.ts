import type { GradientLayerSeed } from "./gradient"
...
const GRADIENT_PRESETS = [
  ...
  id: string
  layers: readonly GradientLayerSeed[]
}>[]

type GradientPreset = (typeof GRADIENT_PRESETS)[number]
type GradientPresetId = GradientPreset["id"]

function resolveGradientPreset(id: GradientPresetId) {
  return (
    GRADIENT_PRESETS.find((preset) => preset.id === id) ?? GRADIENT_PRESETS[0]
  )
}

export { GRADIENT_PRESETS, resolveGradientPreset }
export type { GradientPreset, GradientPresetId }

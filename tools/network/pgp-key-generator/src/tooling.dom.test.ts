...
describe('pgp key generator tool metadata', () => {
  it('exports tool info metadata', () => {
    expect(toolInfo.toolID).toBe('pgp-key-generator')
    expect(toolInfo.path).toBe('/tools/pgp-key-generator')
    expect(toolInfo.features).toContain('offline')
    expect(toolInfo.meta.en.name).toContain('PGP')
